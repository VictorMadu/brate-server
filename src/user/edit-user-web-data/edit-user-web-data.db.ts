...
export class DbService {
    ...
    async _editUserWebData(psql: PoolClient, inData: InData): Promise<boolean> {
        const queryCreator = new EditUserDataQueryCreator({
            ...
            remove_trade_notification_after: this.helper.sanitize(
                inData.remove_trade_notification_after
            ),
            remove_price_alert_notification_after: this.helper.sanitize(
                inData.remove_price_alert_notification_after
            ),
            remove_fund_notification_after: this.helper.sanitize(
                inData.remove_fund_notification_after
            ),
...
class EditUserDataQueryCreator {
    ...
    getQuery() {
      ...
      SET
        ...
        ${this.createColSetQuery(
            ...
            this.sanitizeNotificationRemoval(this.remove_trade_notification_after)
        )},
        ${this.createColSetQuery(
            web_clients.remove_price_alert_notification_after,
            this.sanitizeNotificationRemoval(this.remove_price_alert_notification_after)
        )},
        ${this.createColSetQuery(
            web_clients.remove_fund_notification_after,
            this.sanitizeNotificationRemoval(this.remove_fund_notification_after)
        )},
    ...
    private isAllNull() {
        const allData = [
            ...
            this.bereau_de_change,
            this.dark_mode,
        ];

        for (let i = 0; i < allData.length; i++) {
            if (allData[i] != null) return false;
        }
        return true;
    }

    private sanitizeNotificationRemoval(removeNotificationAfter?: number) {
        if (removeNotificationAfter === -1) return `NULL`;
        return removeNotificationAfter;
    }
