...
import { sellers, users, web_clients } from "../../utils/postgres-db-types/erate";
...
export class DbService {
    ...
    async query(inData: InData) {
        ...
        try {
            const result = await this.runner.runQuery(
                async (psql) => await this.runUserCreationProcessAndGetData(psql, inData)
            );
    ...
    private async runUserCreationProcessAndGetData(psql: PoolClient, inData: InData) {
        const verificationData = await this.createVerificationAndGetData(psql, inData);
        if (!verificationData) return undefined;
        const userData = await this.createUserAndGetData(psql, inData, verificationData);
        if (!userData) return undefined;
        const sellerData = await this.createSeller(psql, userData.user_id);
        if (!sellerData) return undefined;
        const isWebClientCreated = await this.createWebClient(psql, userData.user_id);
