...
export class GetAlertListDbService {
  ...
  private async _getNotificationData(psql: PoolClient, inData: InData) {
    ...
    const totalNotificationFromLastCheck = await this.getTotalFromLastCheck(
      psql,
      inData
    );
  ...
  private async getNotificationMsg(
    ...
    { msg: string; id: string; type: "P" | "F" | "T"; created_at: number }[]
  > {
    console.log(
      "getNotificationMsg query",
      ` SELECT 
  ...
  FETCH FIRST ${inData.pageCount} ROWS ONLY `
    ...
    console.log("getNotificationMsg result", result);
    console.log(
      "getNotificationMsg getAllRows result",
      this.helper.getAllRows(result)
    );
  ...
  private async getTotalFromLastCheck(psql: PoolClient, inData: InData) {
    ...
    const result = await psql.query<{ total: number }>(`
      ...
      WHERE 
        ...
        ${w}.${web_clients.user_id} = ${this.helper.sanitize(
      inData.userId
    )} AND 
  ...
  private createWhereToQuery(to?: number) {
    ...
    return `EXTRACT(EPOCH FROM ${table.created_at}) <= ${this.helper.sanitize(
      to
    )}`;
  }
}
