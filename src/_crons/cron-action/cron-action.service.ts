...
export class CronActionService {
    ...
    private async setAndGetCurrencyNamesInDbWithRetry(): Promise<
        string[] | undefined
    > {
        const diminishingRetry = new DiminishingRetry(
            new MultipleIncr(START_TIME, 2, END_TIME)
        );

        return await diminishingRetry.run(
            async () => await this.setAndGetCurrencyNamesInDb()
        );
    }

    private createCurrencyParallelRateUpdateCron() {
        const cronDiminishingRetry = new DiminishingRetry(
            new MultipleIncr(START_TIME, STEP, END_TIME)
        );
    ...
    private createDeleteExpiredNotificationCron() {
        return new CronJob("50 * * * * *", () => {
            console.log(
                "Running Cron Job `notificationDbService.deleteExpiredNotification`"
            );
    ...
    private getApiKeys() {
        return this.configService.get<"currency_rate.api_keys">(
            "currency_rate.api_keys"
        );
    }
}
