...
-- TODO: Add on uuid_generation conflict, try to re-generate
...
CREATE TABLE IF NOT EXISTS user_verification_details (
  ...
  tried_passwords_at TIMESTAMPTZ[]  DEFAULT ARRAY[]::TIMESTAMPTZ[] 


  -- DOCUMENTATION
  -- the item on index say `i` of `one_time_passwords`, `created_at` and `no_of_retries` corresponds together. Thus the length of each field should be the same
);
...
-- ALTER TABLE user_verification_details ADD FOREIGN KEY(user_id) REFERENCES users(user_id);
