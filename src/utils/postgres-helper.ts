...
abstract class PostgresRunnerBase<P extends Client | PoolClient> {
    ...
    async runQuery<T extends unknown>(queryFn: (psql: P) => Promise<T>): Promise<T | undefined> {
        try {
...
export class PostgresHeplper {
    ...
    getNthRow<T extends QueryResultRow>(queryResult: QueryResult<T>, rowIndex: number) {
