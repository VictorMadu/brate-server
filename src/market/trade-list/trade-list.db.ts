...
export class DbService {
  ...
  private async _getTradersDataAndLastestRates(
    psql: PoolClient,
    inData: InData
  ) {
    const queryCreator = new GetTradersDataAndLastestRatesQueryCreator(
      this.helper,
      inData
    );
    console.log(
      "_getTradersDataAndLastestRates query ",
  ...
  getQuery() {
    ...
    (
      ...
      this.b
        ...
        ${toString(
          `${this.b}.${blackRates.base} || ' ' || ${this.b}.${blackRates.quota}`
        )} AS pair,
        ${toFloat(`${this.t}.${wallet.amount}`)} AS amount_available,
        FIRST_VALUE(${toFloat(`${this.b}.${blackRates.rate}`)}) OVER ${
      this.w
    } AS rate,
        FIRST_VALUE(${timestampToFloat(`${this.b}.${blackRates.time}`)}) OVER ${
      this.w
    } AS created_at
      FROM 
      ...
      this.b
    }.${blackRates.quota}, ${this.b}.${blackRates.seller_id} ORDER BY ${
      this.b
