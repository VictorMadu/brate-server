...
export class DbService {
    ...
    async getWalletAmountData(inData: InData) {
        return this.runner.runQuery((psql) => this._getWalletAmountData(psql, inData));
    }

    private async _getWalletAmountData(psql: PoolClient, inData: InData) {
        const queryCreator = new GetWalletAmountDataQueryCreator(this.helper, inData);
...
class GetWalletAmountDataQueryCreator {
    ...
    private createWhereQueryForSelectSpecificCurrencies() {
        ...
        else return `${transactions.currency_id} IN (${this.helper.sanitize(currencies)})`;
    }
}
