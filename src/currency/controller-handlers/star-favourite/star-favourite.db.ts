  ...
  async addToFavourite(inData: InData): Promise<number> {
    return (
      (await this.runner.runQuery(
        async (psql) => await this.createFavouriteOrUpdateItsFavouritesAtTime(psql, inData)
      )) ?? 0
    );
  }

  async createFavouriteOrUpdateItsFavouritesAtTime(psql: PoolClient, inData: InData) {
    const queryCreator = new FavouriteOrUpdateItsFavouritesAtTimeQueryCreator(inData, this.helper);
